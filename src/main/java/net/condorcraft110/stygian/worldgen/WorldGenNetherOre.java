...
public class WorldGenNetherOre extends WorldGenerator
{
	...
	public WorldGenNetherOre(Block block, int metadata, int numBlocks)
	{
		this.block = block;
			...
			for(int blockX = xStart; blockX <= xStop; blockX++)
			{
				...
				for(int blockY = yStart; blockY <= yStop; blockY++)
				{
					...
					for(int blockZ = zStart; blockZ <= zStop; blockZ++)
					{
						...
						if(d12 * d12 + d13 * d13 + d14 * d14 < 1.0D && (WorldGenHandler.stupidGeneration || world.getBlock(blockX, blockY, blockZ) == Blocks.netherrack))
						{
